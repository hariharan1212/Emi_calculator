    ...
    return(
        <>
            ...
            <div  className='Loaninfo_container d-flex justify-content-center align-items-center mt-3 shadow-lg p-3 mb-5 bg-body-tertiary rounded'>
                ...
                <p className='fw-medium'>Monthly Emi : {monthlyemi ? monthlyemi.toFixed(2) : '--'} </p>
                <p className='fw-medium'>Total Final Amout : {finalamount ? finalamount.toFixed(2) : '--'} </p>
                <p className='fw-medium'>Total Emi : {totalemiamount ? totalemiamount.toFixed(2) : '--'} </p>
                </div>
            </div>
        </>
    )
